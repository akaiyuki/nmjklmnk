...
import com.fasionparade.fasionparadeApp.Functions.Core.ContactItemComparator;
import com.fasionparade.fasionparadeApp.Functions.Object.ContactItemInterface;
import com.fasionparade.fasionparadeApp.Views.SectionIndex.ContactsSectionIndexer;

import java.util.Collections;
import java.util.List;

public class ContactListHelperAdapter extends ArrayAdapter<Contacts> {

	private int resource; // store the resource layout id for 1 row
	private boolean inSearchMode = false;
	private List<Contacts> myFriendArraylist;
	
	private ContactsSectionIndexer indexer = null;
	
	public ContactListHelperAdapter(Context _context, int _resource, List<Contacts> _items, List<Contacts> _myFriendArraylist) {
	...
	public void populateDataForRow(View parentView, ContactItemInterface item , int position){
		...
		View infoView = parentView.findViewById(R.id.infoRowContainer_header);
		TextView nameView = (TextView)infoView.findViewById(R.id.headernameView);
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		//Log.i("ContactListHelperAdapter", "item: " + item.getItemForIndex());
